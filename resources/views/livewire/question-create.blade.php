<x-custom-form-section action="{{ route('questionnaires.questions.store', $questionnaire) }}">
    ...
    <x-slot name="form">
        ...
        <div class="col-span-6 sm:col-span-4">
            ...
            <x-jet-input id="question" type="text" class="mt-1 block w-full" name="question[question]" value="{{ old('question.question') }}" />
                ...
                <div class="flex items-center space-x-4">
                    <div class="flex-1">
                        <x-jet-input id="answer-{{ $i + 1 }}" type="text" class="mt-1 block w-full" 
                            ...
                            wire:model.lazy="answers.{{ $i }}"
                            placeholder="Option {{ $i + 1 }}" />
                        <x-jet-input-error for="answers.{{ $i }}.answer" class="mt-2" />
                    </div>
                    <div>
                        <button type="button" class="text-red-500 focus:outline-none" wire:click.prevent="deleteAnswerField({{ $i }})">Remove</button>
                    </div>
                </div>
            </div>                
            @endforeach
